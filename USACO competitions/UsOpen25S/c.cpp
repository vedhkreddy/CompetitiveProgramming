...
void dfsmax(int node, int parent){
    ...
    else if (difficulty[node] > dp[parent][1]) dp[node][1] = difficulty[node];
    else{
        dp[node][1] = dp[parent][1];
    }
    if (parent != -1) depth[node] = depth[parent] + 1;
    for (int i : adj[node]){
        if (!visited[i]) dfsmax(i, node);
    }
}
...
void dfsfill1(int node, priority_queue<int> p){
    pqi maxes = p;
    if (maxes.size() < cur){
        dp[node][cur] = 0;
        maxes.push(difficulty[node]);
        dfsfill1(node, maxes);
        return;
    }
    if (maxes.top() < difficulty[node]){
        maxes.pop(); maxes.push(difficulty[node]);
    }
    dp[node][cur] = maxes.top();
    for (int x : adj[node]){
        if (!visited[x]) dfsfill1(x, maxes);
    }
}
